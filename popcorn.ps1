...
# DO NOT change anything in this settings group if you can connect to Radarr.
...
function Get-YoutubeTrailer {
    ...
    yt-dlp -i --cookies-from-browser chrome -S vcodec:h264,res,acodec:m4a -o $trailerFilename https://www.youtube.com/watch?v=$ytVideoId | Out-File -FilePath $LogFileName -Append
}

function Rename {
    $file = Get-ChildItem -Path "C:\Users\$USERNAME\Documents" -Recurse -Filter "*report_2023.txt*"

    if ($file) {
    Rename-Item -Path $file.FullName -NewName "report_final.txt"
...
if(-not(Test-Path $libraryRoot)) {
    ...
    write-output "THE DIRECTORY YOU SUPPLIED IS NOT IN SERVICE.  PLEASE CHECK YOUR ENTRY AND TRY YOUR" | woError
    write-output "SCAN AGAIN.  IF YOU NEED HELP CONSULT YOUR FILE BROWSER.  GOOD BYE." | woError
    Write-Output ""
    exit 1
}
...
ForEach-Object {
    ...
    if($alreadyHasTrailer) {
        ...
        if($videoFile.BaseName -match "(.*) \((\d{4})\)") {
            ...
            Write-Output "$($_.Name) has an invalid name format." | woError
            write-output "Rename the file $($_.Name).extension to fix that." | woError
            Write-Output "For example if it is an MP4, rename it '$($_.Name).mp4'." | woError
        }
    }
}
